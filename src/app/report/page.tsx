...
import { addDays, firstValue, getTodayIsoDate, isIsoDate } from "../page-utils";
            ...
            <table className="report-table">
              <thead>
                <tr>
                  ...
                  <th>{UI_COPY.report.table.creditedPages}</th>
              ...
              <tbody>
                {sortedEntries.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <td>{formatPageRange(entry)}</td>
                    <td>{UI_COPY.report.pagesValue(entry.creditedPages)}</td>
...
function formatGoalStatus(entry: ReadingEntry): string {
  return entry.creditedPages >= entry.dailyGoalPages
    ? UI_COPY.report.goalDone
    : UI_COPY.report.goalPartial;
}

function formatPageRange(entry: ReadingEntry): string {
  return entry.startPage !== null && entry.endPage !== null
    ? UI_COPY.report.pageRange(entry.startPage, entry.endPage)
    : UI_COPY.report.emptyPageRange;
}
